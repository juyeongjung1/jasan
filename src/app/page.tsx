...
export default function DashboardPage() {
  // 多言語 (韓国語メイン / 日本語切替) & プライバシー保護 (金額マスク)
  ...
  useEffect(() => {
    ...
    if (hasUpdated) {
      ...
      setNotificationMsg(lang === 'ko' ? `🎉 적립 지정일을 맞이하여 ${generatedLogs.length}건의 적립이 자산에 자동 반영되었습니다!` : `🎉 積立指定日を迎えました！ ${generatedLogs.length}件の積立（計¥${totalAdd.toLocaleString()}）を資産に自動反映しました。`);
    } else {
  ...
  const handleDeleteHolding = (id: string) => {
    if (confirm(lang === 'ko' ? '이 자산을 삭제하시겠습니까?' : 'この資産を削除してもよろしいですか？')) {
  ...
  const handleDeleteRecurringPlan = (id: string) => {
    if (confirm(lang === 'ko' ? '이 적립 설정을 삭제하시겠습니까?' : 'この積立設定を削除しますか？')) {
  ...
  const handleExecuteManual = (planId: string) => {
    ...
    if (log) {
      ...
      setNotificationMsg(lang === 'ko' ? `⚡「${log.holdingName}」에 적립이 가산 반영되었습니다!` : `⚡「${log.holdingName}」に ¥${log.amountJpy.toLocaleString()} の積立を加算反映しました！`);
    }
  };
